    ...
    <Col sm={12} md={6} lg={4}>
      <Card className="mx-2 my-2">
        <Card.Body className="p-0 pb-3">
          <Card.Title className="px-4 pt-3" ><Link to={`/detalle/${service?.id}`}>{service?.name}</Link></Card.Title>
          <Card.Text className="px-4 text-truncate-2">{service?.description}</Card.Text>
          {showFooter && (
